...
for (week in 1:length(unique(df$week))) { # unique weeks
    ...
    for (play in unique(temp$playId)) { 
      ...
      for (frame in unique(dat$frameId)) {
        ...
        if(nrow(out)>0) { # sometimes there is no ball snap in the data, so out will be null
          ...
          # revisualization of the player betweenness metrics 
          V(g)$betw <- round(betw, 3)
          plot(g, layout=l, vertex.label=V(g)$betw)
          ...
          if (week == 1 & game==2021091207 & play==3828) {
            a <- out %>% dplyr::select(week, gameId, playId, frameId, event) %>% unique(.) %>% filter(complete.cases(.))
            b <- cbind(a, nodes, l)
            field <- rbind(example, b)
          }
        }
        break
        # write.csv(field, '../use_case/graph.csv')
      }
      break
    }
  ...
  #   left_join(plays, by = c('gameId', 'playId')) %>%
  #   left_join(games, by = c('week', 'gameId')) %>%
  #   write.csv(., paste0('../builds/build_',week,'.csv'), row.names = FALSE)
}